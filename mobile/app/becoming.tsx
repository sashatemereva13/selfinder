...
import { BECOMING_STAGES } from '../src/content/journeys/becoming';
import { JourneySessionDTO, JourneyPurchase } from '../src/types';
import { useAppAccentRgb } from '../src/utils/appAccent';

const KALEIDOSCOPE_LOADING_SIZE = 300;

// Becoming — "who am I becoming?" Ninth Journey built on the engine (see
// docs/journeys-concept.md). Structurally identical to the prior
// conversational-only Journeys — no custom renderStageInput.
export default function BecomingScreen() {
  ...
  const hookPurchases = useJourneyPurchases('becoming');
  ...
  useEffect(() => {
    ...
    purchaseJourney('becoming', session.token)
  ...
  const handleDoItAgain = async () => {
    ...
    setRestarting(true);
    try {
      const purchase = await purchaseJourney('becoming', session.token);
    ...
    <View style={[styles.root, { paddingTop: insets.top + spacing[4] }]}>
      ...
      {!session ? (
        <ScrollView contentContainerStyle={styles.teaserContent}>
          <Text style={styles.title}>{t('products.becomingLabel')}</Text>
