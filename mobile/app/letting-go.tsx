...
import { LETTING_GO_STAGES } from '../src/content/journeys/lettingGo';
import { JourneySessionDTO, JourneyPurchase } from '../src/types';
import { useAppAccentRgb } from '../src/utils/appAccent';

const KALEIDOSCOPE_LOADING_SIZE = 300;

// Letting Go — "what am I actually holding onto?" Sixth Journey built on
...
export default function LettingGoScreen() {
  ...
  const hookPurchases = useJourneyPurchases('letting-go');
  ...
  useEffect(() => {
    ...
    purchaseJourney('letting-go', session.token)
  ...
  const handleDoItAgain = async () => {
    ...
    setRestarting(true);
    try {
      const purchase = await purchaseJourney('letting-go', session.token);
    ...
    <View style={[styles.root, { paddingTop: insets.top + spacing[4] }]}>
      ...
      {!session ? (
        <ScrollView contentContainerStyle={styles.teaserContent}>
          <Text style={styles.title}>{t('products.lettingGoLabel')}</Text>
