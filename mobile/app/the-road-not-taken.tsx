...
import { THE_ROAD_NOT_TAKEN_STAGES } from '../src/content/journeys/theRoadNotTaken';
...
// been?" Fifth Journey built on the engine (see docs/journeys-concept.md).
// Structurally identical to the-choice.tsx/the-loop.tsx/whose-voice.tsx —
// no custom renderStageInput, every stage is a plain conversational
// exchange.
export default function TheRoadNotTakenScreen() {
  ...
  const hookPurchases = useJourneyPurchases('the-road-not-taken');
  ...
  useEffect(() => {
    ...
    purchaseJourney('the-road-not-taken', session.token)
  ...
  const handleDoItAgain = async () => {
    ...
    setRestarting(true);
    try {
      const purchase = await purchaseJourney('the-road-not-taken', session.token);
    ...
    <View style={[styles.root, { paddingTop: insets.top + spacing[4] }]}>
      ...
      {!session ? (
        <ScrollView contentContainerStyle={styles.teaserContent}>
          <Text style={styles.title}>{t('products.theRoadNotTakenLabel')}</Text>
        ...
        <JourneyWizard
          journey="the-road-not-taken"
          purchaseId={mostRecentPurchase.id}
          stages={THE_ROAD_NOT_TAKEN_STAGES}
