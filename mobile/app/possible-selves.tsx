...
import { POSSIBLE_SELVES_STAGES } from '../src/content/journeys/possibleSelves';
...
// Journey built on the engine (see docs/journeys-concept.md). Structurally
// identical to the prior conversational-only Journeys — no custom
// renderStageInput.
export default function PossibleSelvesScreen() {
  ...
  const hookPurchases = useJourneyPurchases('possible-selves');
  ...
  useEffect(() => {
    ...
    purchaseJourney('possible-selves', session.token)
  ...
  const handleDoItAgain = async () => {
    ...
    setRestarting(true);
    try {
      const purchase = await purchaseJourney('possible-selves', session.token);
    ...
    <View style={[styles.root, { paddingTop: insets.top + spacing[4] }]}>
      ...
      {!session ? (
        <ScrollView contentContainerStyle={styles.teaserContent}>
          <Text style={styles.title}>{t('products.possibleSelvesLabel')}</Text>
        ...
        <JourneyWizard
          journey="possible-selves"
          purchaseId={mostRecentPurchase.id}
          stages={POSSIBLE_SELVES_STAGES}
