...
import { ENOUGH_STAGES } from '../src/content/journeys/enough';
...
// Journey in the current catalog, built on the engine (see docs/journeys-
// concept.md). Structurally identical to the prior conversational-only
// Journeys — no custom renderStageInput.
export default function EnoughScreen() {
  ...
  const hookPurchases = useJourneyPurchases('enough');
  ...
  useEffect(() => {
    ...
    purchaseJourney('enough', session.token)
  ...
  const handleDoItAgain = async () => {
    ...
    setRestarting(true);
    try {
      const purchase = await purchaseJourney('enough', session.token);
    ...
    <View style={[styles.root, { paddingTop: insets.top + spacing[4] }]}>
      ...
      {!session ? (
        <ScrollView contentContainerStyle={styles.teaserContent}>
          <Text style={styles.title}>{t('products.enoughLabel')}</Text>
