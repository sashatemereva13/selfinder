...
import { THE_MIRROR_STAGES } from '../src/content/journeys/theMirror';
...
// renderStageInput.
export default function TheMirrorScreen() {
  ...
  const hookPurchases = useJourneyPurchases('the-mirror');
  ...
  useEffect(() => {
    ...
    purchaseJourney('the-mirror', session.token)
  ...
  const handleDoItAgain = async () => {
    ...
    setRestarting(true);
    try {
      const purchase = await purchaseJourney('the-mirror', session.token);
    ...
    <View style={[styles.root, { paddingTop: insets.top + spacing[4] }]}>
      ...
      {!session ? (
        <ScrollView contentContainerStyle={styles.teaserContent}>
          <Text style={styles.title}>{t('products.theMirrorLabel')}</Text>
