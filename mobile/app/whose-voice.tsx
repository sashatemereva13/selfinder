...
import { WHOSE_VOICE_STAGES } from '../src/content/journeys/whoseVoice';
import { JourneySessionDTO, JourneyPurchase } from '../src/types';
import { useAppAccentRgb } from '../src/utils/appAccent';

const KALEIDOSCOPE_LOADING_SIZE = 300;

// Whose Voice? — "is this really what I believe or want?" Fourth Journey
// built on the engine (see docs/journeys-concept.md). Structurally
// identical to the-choice.tsx/the-loop.tsx — no custom renderStageInput.
// The doc's own "bring the others into the room" visual primitive (the
// belief centered, each voice arranged around it) is deliberately not
// built yet — see whoseVoice.ts's own 2026-09-03 comment; ORIGIN gathers
// the same material as plain conversational text for now.
export default function WhoseVoiceScreen() {
  ...
  const hookPurchases = useJourneyPurchases('whose-voice');
  ...
  useEffect(() => {
    ...
    purchaseJourney('whose-voice', session.token)
  ...
  const handleDoItAgain = async () => {
    ...
    setRestarting(true);
    try {
      const purchase = await purchaseJourney('whose-voice', session.token);
    ...
    <View style={[styles.root, { paddingTop: insets.top + spacing[4] }]}>
      ...
      {!session ? (
        <ScrollView contentContainerStyle={styles.teaserContent}>
          <Text style={styles.title}>{t('products.whoseVoiceLabel')}</Text>
