...
import { THE_THRESHOLD_STAGES } from '../src/content/journeys/theThreshold';
...
// renderStageInput.
export default function TheThresholdScreen() {
  ...
  const hookPurchases = useJourneyPurchases('the-threshold');
  ...
  useEffect(() => {
    ...
    purchaseJourney('the-threshold', session.token)
  ...
  const handleDoItAgain = async () => {
    ...
    setRestarting(true);
    try {
      const purchase = await purchaseJourney('the-threshold', session.token);
    ...
    <View style={[styles.root, { paddingTop: insets.top + spacing[4] }]}>
      ...
      {!session ? (
        <ScrollView contentContainerStyle={styles.teaserContent}>
          <Text style={styles.title}>{t('products.theThresholdLabel')}</Text>
        ...
        <JourneyWizard
          journey="the-threshold"
          purchaseId={mostRecentPurchase.id}
          stages={THE_THRESHOLD_STAGES}
