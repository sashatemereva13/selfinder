        ...
        <div className="sf-navSectionBody" role="list">
          {items.map((item) => {
            ...
            return (
              ...
              >
                {icon && (
                  <span className="sf-roomIcon" aria-hidden="true">
                    {icon}
                  </span>
                )}
                <span className="sf-roomBody">
                  <span className="sf-roomTop">
                    ...
                    {item.stage && (
                      <span className="sf-roomStage">{item.stage}</span>
                    )}
                  </span>
                  <span className="sf-roomDesc">{item.description}</span>
                </span>
                {isCurrent && (
                  <span className="sf-roomDot" aria-hidden="true" />
                )}
              </Link>
            );
          })}
...
// ─── Overlay nav — Threshold + Instruments ───────────────────────────────────
function OverlayNav({ title, subtitle }) {
  ...
  return (
    <section
      className="sf-journeyNav sf-journeyNav-overlay"
      aria-label="Selfinder navigation"
    >
...
function StandardNav({ currentKey, variant, title, subtitle, primaryAction }) {
  ...
  const currentStep =
    ...
    HOUSE_ROOMS.find((r) => r.key === currentKey) ??
    null;
